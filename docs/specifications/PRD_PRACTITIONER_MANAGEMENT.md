...
- [ ] 表單包含欄位：
  ...
  - 可被預約的課程（必填，至少選一個）
  - 個人簡介、照片（可選）
- [ ] 顏色色盤提供 8+ 種顏色選項，已選顏色需視覺化標示
- [ ] 課程清單動態從課程管理模組載入
- [ ] 可多選課程，已選課程需視覺化標示
- [ ] 驗證：姓名非空、顏色已選、至少選一個課程
- [ ] 提交後立即在列表中顯示新增的從業人員（含顏色標識）
- [ ] 行事曆和甘特圖立即顯示該老師的預約（使用選定顏色）
- [ ] 新增成功後顯示「新增成功」提示訊息
...
- [ ] 表單預填現有資訊：
  ...
  - **已指派的課程需被勾選顯示**
- [ ] 可修改所有欄位（除 ID 外）
- [ ] 修改顏色後，行事曆和甘特圖立即更新該老師預約的顏色
- [ ] 課程清單動態載入，可新增或移除課程指派
- [ ] 至少保持一個課程被指派
- [ ] 提交後立即更新列表和預約頁面
- [ ] 預約頁面同步顯示更新後的名稱、顏色和可被預約的課程
- [ ] 編輯成功後顯示「更新成功」提示訊息
...
**使用者故事**：
身為管理員，我想為每位老師分配不同的顏色，使其在行事曆和甘特圖上可以快速視覺化識別。

**驗收準則**：
- [ ] 新增/編輯老師時，可從顏色色盤選擇（8+ 種顏色）
- [ ] 已選顏色需視覺化標示（色彩方塊 + 顏色名稱）
- [ ] 顏色是必選欄位，不能留空
- [ ] 不同老師可分配同一顏色（無唯一性限制）
- [ ] 行事曆視圖中，該老師的預約卡片使用選定顏色作為背景或邊框
- [ ] 甘特圖視圖中，該老師的預約色塊使用選定顏色
- [ ] 修改老師顏色後，所有視圖立即更新
- [ ] 從業人員列表中老師名稱前的狀態指示圖示使用該顏色
...
提交表單
  ↓
行事曆和甘特圖中王美麻的預約卡片顯示為紫色
  ↓
列表中王美麻的圓形指示圖也是紫色
```

**技術需求**：
- 資料模型：`practitioners` 表添加 `color_hex` 欄位（VARCHAR，存儲 HEX 色值）
- 預設色盤：
  ```
  紅色 (#EF4444)、橙色 (#F97316)、黃色 (#FBBF24)
  綠色 (#10B981)、青色 (#06B6D4)、藍色 (#3B82F6)
  紫色 (#9333EA)、粉色 (#EC4899)
  ```
- CalendarPage：使用 `practitioner.color_hex` 渲染預約卡片背景色
- GanttPage：使用 `practitioner.color_hex` 渲染預約色塊
- 列表視圖：使用 `practitioner.color_hex` 渲染狀態指示圖示
- Realtime 訂閱：顏色變更時自動同步所有視圖
...
CREATE TABLE practitioners (
  ...
  CHECK (color_hex ~ '^#[0-9A-Fa-f]{6}$')  -- 驗證 HEX 色值格式
);
```
...
**顏色標識區塊**：
- 提供 8 種預設顏色：紅(#EF4444)、橙(#F97316)、黃(#FBBF24)、綠(#10B981)、青(#06B6D4)、藍(#3B82F6)、紫(#9333EA)、粉(#EC4899)
- 使用圓形色塊表示，已選顏色顯示勾選標記或邊框
- 必須選擇一個顏色
- 顯示已選顏色的名稱和 HEX 值
...
- **顏色標識管理**：預設色盤、顏色選擇、視覺化顯示
- **顏色在視圖中的應用**：行事曆預約卡片、甘特圖色塊、列表指示圖
