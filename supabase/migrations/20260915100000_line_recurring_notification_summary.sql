-- ============================================================
-- LINE 循環預約摘要
-- 將 outbox 內的系列堂數交給 Worker，讓單一卡片清楚標示批次範圍。
-- ============================================================
...
AS $$
  ...
  SELECT
    ...
    CASE
      WHEN (claimed.payload_snapshot ->> 'recurrence_count') ~ '^[0-9]+$'
        THEN (claimed.payload_snapshot ->> 'recurrence_count')::INTEGER
      ELSE NULL
    END
  FROM claimed
...
COMMENT ON FUNCTION public.claim_line_notification_jobs(INTEGER) IS
  'service_role 專用：原子領取 LINE 通知，並回傳商品券與循環預約摘要。';
