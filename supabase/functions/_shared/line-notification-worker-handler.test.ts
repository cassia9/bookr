...
function validJob(
  overrides: Partial<LineNotificationJob> = {},
): LineNotificationJob {
  return {
    ...
    template_content:
      "{{store_name}}：{{customer_name}} 的 {{service_name}} 已確認，老師 {{practitioner_name}}，時間 {{start_time}}",
    customer_name: "王小明",
    service_name: "伸展課",
...
Deno.test("Worker 組版發送並完成有效確認通知", async () => {
  ...
  assert(
    serializedMessage.includes('"type":"flex"'),
    "預約通知應使用 Flex Message",
  );
...
Deno.test("LINE Push API 可發送 Flex Message", async () => {
  ...
  assert(
    payload.messages[0].contents.type === "bubble",
    "預約卡片應使用 bubble",
  );
});

Deno.test("Worker 在預約卡片顯示商品券與使用後餘額", async () => {
  let sentMessage: unknown = null;
  const setup = dependencies([
    validJob({
      voucher_product_name: "進階修復五堂券",
      voucher_service_remaining: 4,
    }),
  ], {
    send: async (options: { message: unknown }) => {
      sentMessage = options.message;
      return { requestId: "line-voucher-request-1" };
    },
  });
  ...
  assert(body.sent === 1, "商品券預約通知應成功送出");
...
Deno.test("Worker 可發送不綁定預約的管理員測試推播", async () => {
  ...
  assert(
    sentMessage === job.template_content,
    "測試推播只能使用後端建立的固定訊息",
  );
