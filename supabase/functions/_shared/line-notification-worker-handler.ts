...
function skipReason(job: LineNotificationJob) {
  ...
  if (
    job.event_type === "booking_received" && job.booking_status !== "pending"
  ) {
    return "booking_state_changed";
  }
  if (
    job.event_type === "booking_confirmed" && job.booking_status !== "confirmed"
  ) {
    return "booking_state_changed";
  }
  if (
    job.event_type === "booking_cancelled" && job.booking_status !== "cancelled"
  ) {
    return "booking_state_changed";
  }
  if (
    ...
    for (const job of jobs) {
      ...
      try {
        const templateValues = {
          ...
          voucher_name: job.voucher_product_name?.trim() || "",
        ...
        const httpStatus = error instanceof LineMessagingError
          ? error.httpStatus
          : null;
