...
function bookingCardIntro(renderedText: string) {
  const detailLinePattern =
    /^(課程|老師|時間|原預約時間|新時間|商品券|剩餘堂數)\s*[：:]/;
  return renderedText
    .split("\n")
    .map((line) => line.trim())
...
function parseRetryAfter(response: Response) {
  ...
  return Math.max(
    1,
    Math.min(86_400, Math.ceil((timestamp - Date.now()) / 1000)),
  );
}
...
export async function fetchLineBotInfo(
  ...
  const botUserId = typeof payload.userId === "string"
    ? payload.userId.trim()
    : "";
  const displayName = typeof payload.displayName === "string"
    ? payload.displayName.trim()
    : "";
  const basicId = typeof payload.basicId === "string"
    ? payload.basicId.trim()
    : "";
...
export function renderLineMessageTemplate(
  ...
  for (const variable of LINE_TEMPLATE_VARIABLES) {
    const safeValue = String(values[variable as LineTemplateVariable] ?? "")
      .trim();
...
export async function verifyLineWebhookSignature(
  ...
  if (!rawBody || !normalizedSignature || normalizedSecret.length < 20) {
    return false;
  }
